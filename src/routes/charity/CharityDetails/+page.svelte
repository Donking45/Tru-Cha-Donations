...
import { json } from "@sveltejs/kit";
    ...
    async function addDetails(event){
      ...
      try {
        const data = await fetch('/charity/CharityDetails',{
          method: 'POST',
           headers: {
          'Content-Type': 'application/json',
        },
        
        body: JSON.stringify({financialInfo, mission,  projects})
  },
)
...
<Card.Root>
  ...
  <Card.Content class='flex flex-row justify-between' >
    <form class="flex flex-col w-[40%]" on:submit={addDetails} >
      ...
      <input bind:value={projects} class="m-2  shadow-sm shadow-black rounded-sm border-[1px] border-black py-[4px] px-[4px]" type="text">

      <label for="">Mission</label>
      <input bind:value={mission} class="m-2 shadow-sm shadow-black rounded-sm  border-[1px] border-black py-[4px] px-[4px]"type="text">

      <label for="">financial Info</label>
      <input bind:value={financialInfo} class="m-2 shadow-sm shadow-black rounded-sm  border-[1px] border-black py-[4px] px-[4px]" type="text">
